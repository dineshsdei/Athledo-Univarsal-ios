...
- (void)reportStatus
{
 //   NSUInteger total = [self.prefetchURLs count];
  //  STLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
 //   STLog(@"Prefetched %d out of %d", _finishedCount, [self.prefetchURLs count]);
...
{
    _finishedCount++;
  //  STLog(@"Prefetched %d out of %d (Failed)", _finishedCount, [self.prefetchURLs count]);
