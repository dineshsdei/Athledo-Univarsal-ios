...
//   if ( isDate==TRUE) {
//        [self setToolbarVisibleAt:CGPointMake(160, toolBarPosition-30)];
//        //isDate=FALSE;
//    }
//    
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    ////NSLog(@"place holder %@",textField.placeholder);
     isDate=FALSE;
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    }else if ([[_obj valueForKey:@"Workout Type"] isEqualToString:@"Lift"])
    {
     ...
     int strTraverselength=(int)textField.text.length;
        ...
        //////check count
//        const char *cp = [value UTF8String];
//        
...
//
//    [m_TableView setContentOffset:CGPointMake(0, point.y + (txt.frame.origin.y+txt.frame.size.height)-60) animated: YES];
//
//}
...
//        NSDictionary *dic=[[NSDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithBool:isRate],@"Rate",[NSNumber numberWithBool:isTime],@"Time",[NSNumber numberWithBool:isDistance],@"Distance", nil];
//        
...
//   [arrRateTimeDistanceStatus addObject:arrtemp];
//    arrtemp=nil;
...
//        }
    ...
    if (TotalDistance > 0 && DistanceCount > 0) {
       ...
       // float b=TotalDistance%DistanceCount;
      //  DistanceAVG=a+b;
...
-(void)setPickerVisibleAt:(CGPoint)point
{
    ...
    if (currentText.text.length > 0) {
            ...
            for (int i=0; i< arrTime.count; i++) {
                ...
                if ([[arrTime objectAtIndex:i] isEqual:currentText.text]) {
